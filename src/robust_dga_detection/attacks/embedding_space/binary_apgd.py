...
class BinaryAPGD(Attack):
    r"""APGD modified to be applicable to binary classification tasks.
    ...
    https://github.com/Harry24k/adversarial-attacks-pytorch/blob/c4da6a95546283992a3d1816ae76a0cd4dfc2d8b/torchattacks/attacks/apgd.py#L11
    ...
    Arguments:
        ...
        loss (str): loss function optimized. ['ce', 'cw'] (Default: 'ce')
    ...
    def __init__(
        ...
        :param eot_iter number of iteration for EO
        :param rho parameter for step-size update
    ...
    def forward(self, images, labels):
        ...
        _, adv_images = self.__perturb(images, labels, cheap=True)

        return adv_images

    def __check_oscillation(self, x, j, k, y5, k3=0.75):
    ...
    def __check_shape(self, x):
        return x if len(x.shape) > 0 else np.expand_dims(x, 0)

    def __cw_loss(self, x, y):
        return -x[np.arange(x.shape[0]), y] + x[np.arange(x.shape[0]), 1 - y]

    def __attack_single_run(self, x_in, y_in):
        ...
        if self.loss == "ce":
            ...
            criterion_indiv = self.__cw_loss
        else:
        ...
        for i in range(self.steps):
            ...
            if self.verbose:
                print(f"iteration: {i} - Best loss: {loss_best.sum():.6f}")
            ...
            with torch.no_grad():
                ...
                if counter3 == k:
                    fl_oscillation = self.__check_oscillation(
    ...
    def __perturb(self, x_in, y_in, best_loss=False, cheap=True):
        ...
        if not best_loss:
                ...
                for counter in range(self.n_restarts):
                    ...
                    if ind_to_fool.numel() != 0:
                        ...
                        (
                            ...
                            loss_curr,
                            adv_curr,
                        ) = self.__attack_single_run(x_to_fool, y_to_fool)  # nopep8
            ...
            for counter in range(self.n_restarts):
                best_curr, _, loss_curr, _ = self.__attack_single_run(x, y)
